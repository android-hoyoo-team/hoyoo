...
public class CircularProgressDrawable extends Drawable {
	...
	public void draw(Canvas canvas) {
		...
		float outerRadius = ((size / 2) * 0.9f) * 0.937f;
		float innerRadius = ((size / 2) * 0.75f) * 0.9f;
	 ...
	 * @param progress Sets the progress
	 */
	private AnimatorSet progress_animation;
	public void setProgress_(float progress) {
		if(progress_animation!=null)
		{
			progress_animation.cancel();
		}
		progress_animation = new AnimatorSet();
		...
		progress_animation.playTogether(progressAnimation);
	...
	private AnimatorSet circle_scale_animation;
	public void setCircleScale_(float circleScale) {
		if(circle_scale_animation!=null)
		{
			circle_scale_animation.cancel();
		}
		circle_scale_animation = new AnimatorSet();
		float old = this.getCircleScale();

		//		ObjectAnimator progressAnimation = ObjectAnimator.ofFloat(this, CircularProgressDrawable.PROGRESS_PROPERTY, old, circleScale);//0f1f
		...
		circle_scale_animation.playTogether(innerCircleAnimation);
		circle_scale_animation.start();
		//      Animator innerCircleAnimation = ObjectAnimator.ofFloat(drawable, CircularProgressDrawable.CIRCLE_FILL_PROPERTY, empty, full);//0f1.2f
		//      innerCircleAnimation.setDuration(1800);
		//      innerCircleAnimation.setInterpolator(new AccelerateDecelerateInterpolator());
	}
